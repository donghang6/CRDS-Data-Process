...
MATS_MULTI_ROOT = PROJECT_ROOT / "output" / "results" / "mats_multi"
...
def step4_multi_fit(etalon_root: Path | None = None,
    ...
    """Step 4: 根据 Step 3 单光谱拟合结果筛除线强离群点，
    用剩余的光谱数据做多光谱联合拟合。
    ...
    etalon_root = etalon_root or ETALON_ROOT
    mats_root = mats_root or MATS_ROOT
    mats_multi_root = mats_multi_root or MATS_MULTI_ROOT
    ...
    for t_dir in sorted(mats_root.iterdir()):
        ...
        # ---- 2. 基于线强的 MAD 筛选 ----
        sw_df["deviation"] = np.abs(sw_df["sw"] - median_sw) / (mad_sw if mad_sw > 0 else 1)
        ...
        for _, r in kept.iterrows():
            csv_path = etalon_root / transition / r["pressure"] / "tau_etalon_corrected.csv"
            if csv_path.exists():
        ...
        try:
            ...
            if result and result.summary_df is not None and not result.summary_df.empty:
                fitter.plot_result(
                    result, multi_out,
...
def _save_multi_fit_summary(result, output_dir: Path,
    ...
    baseline_df = result.baseline_linelist
    ...
    # 同时保存到 final 目录
    final_dir = FINAL_ROOT / transition
...
def main():
    ...
    # ---- Step 3 ----
    step3_mats(lineprofile=lineprofile)

    # ---- Step 4 ----
    step4_multi_fit(lineprofile=lineprofile, sw_sigma=sw_sigma)
